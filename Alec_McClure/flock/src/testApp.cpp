...
void testApp::setup(){
    ...
    for (int i=0; i<200; i++) {
        Flocker f;
        flockers.point = ofPoint(ofRandomWidth(), ofRandomHeight());
        f.speed = 1;
        f.angle = 0;
...
void testApp::update(){
    ...
    */
    ...
    for (int i=0; i<flockers.size(); i++) {
        ...
        //flockers[i].point.x += cos(flockers[i].angle)*flockers[i].speed;
        //flockers[i].points.y += sin(flockers[i].angle)*flockers[i].speed;
    }
}
