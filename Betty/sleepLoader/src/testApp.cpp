...
void testApp::setup() {
    ...
    int nFiles = dir.listDir("mirror1");
    if(nFiles) {
...
void testApp::update() {
    ...
    if (bBrushDown){
        for (int i=0; i<people.size(); i++){
            ...
            brush.draw(centroid.x-150, centroid.y-100);
        }
    }
    maskFbo.end();
...
void testApp::draw() {
  ...
  //  hand.draw(0, 0); //top image
    ...
    for (int i=0; i<people.size(); i++){
        ...
        centroid.x *= ofGetWidth();
        centroid.y *= ofGetHeight();
        brush.draw(centroid.x-100, centroid.y);
    }
