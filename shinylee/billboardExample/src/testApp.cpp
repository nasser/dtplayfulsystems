...
void testApp::setup() {
	...
	for (int i=0; i<NUM_BILLBOARDS; i++) {
		...
		billboards.getColors()[i].set(ofColor::fromHsb(ofRandom(0, 480), 255, 255));
	    billboardSizeTarget[i] = ofRandom(4, 70);
...
void testApp::update() {
	...
	float t = (ofGetElapsedTimef()) * 0.6f;
	float div = 250.0;
...
//--------------------------------------------------------------
void testApp::draw() {
	ofBackgroundGradient(ofColor(255), ofColor(240, 173, 233));
